   ...
   extends \damix\engines\tools\GeneratorContent
{
	...
	public function generate( EventSelector $selector ) : bool
    {
        if( $this->open( $selector ) )
        {
            $this->_document = \damix\engines\tools\xmlDocument::createDocument('events');
			...
			$this->tab( 1 );
			$this->writeLine( 'extends \damix\engines\events\EventBase' );
			$this->tab( -1 );
			$this->writeLine( '{' );
            $this->writecontent();
			$this->writeLine( '}' );
    ...
    protected function open( EventSelector $selector ) : bool
    {
	...
	{
		$dom = $this->_document;
        $sections = $dom->xPath( '/events/listener' );
        
		
        $content = array();
		
		foreach( $sections as $listener )
		{
			$selector = $dom->getAttribute( $listener, 'selector' );
			foreach( $listener->childNodes as $event )
			{
				$content[] = '$this->events[\''. $dom->getAttribute($event, 'name') .'\'][] = \'' . $selector . '\';';
			}
		}
